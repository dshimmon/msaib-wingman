# Decision Index

> Generated from authoritative decision metadata. Do not edit directly.

| Decision | Namespaces | Status | Date | Record |
|---|---|---|---|---|
| `ARCH-001` | wingman-os, atlas, radar | accepted | 2026-07-31 | [Wingman Airframe Boundaries](architecture/airframe-boundaries.md) |
| `ARCH-002` | atlas, radar, wingman-os | accepted | 2026-08-07 | [Product Separation](architecture/product-separation.md) |
| `ARCH-003` | wingman-os, atlas, radar | accepted | 2026-08-01 | [Source Traceability](architecture/source-traceability.md) |
| `ARCH-004` | wingman-os, atlas | accepted | 2026-08-07 | [Compatibility Facades](architecture/compatibility-facades.md) |
| `DATA-001` | wingman-os, security, data | accepted | 2026-08-01 | [Ledger and Data Safety Boundary](security/ledger-and-data-safety.md) |
| `GOV-001` | governance | accepted | 2026-08-07 | [Repository Records and Lifecycle](governance/repository-records.md) |
| `GOV-002` | governance, wingman-os, radar | accepted | 2026-08-01 | [Approved Roadmap Sequencing](governance/roadmap-sequencing.md) |
| `GOV-003` | governance | accepted | 2026-08-07 | [Historical Mission Ratification and Record Authority](governance/historical-mission-ratification.md) |
| `GOV-004` | governance, operations | accepted | 2026-08-09 | [Crew Chief Independent Audit Workflow](governance/crew-chief-audit.md) |
| `OPS-001` | operations, governance | accepted | 2026-08-01 | [Development Flightline Operating Model](governance/development-flightline.md) |
