...
from wingman.core.prompt_optimizer import optimize_prompt
...
with st.sidebar:
    ...
    if uploaded_files:
        ...
        course_overrides = {}
        st.caption(
            "Review every file. Leave a course override blank to use "
            "the confirmed batch course ID."
        )
        for file_index, uploaded_file in enumerate(uploaded_files):
            ...
            display_name_overrides.setdefault(
                uploaded_file.name,
                entered_display_name,
            )
            course_overrides.setdefault(
                uploaded_file.name,
                entered_course_override,
            )
        ...
        selection_changed = (
            st.session_state.get("batch_selection_signature")
            != selection_signature
        )
        ...
        batch_inputs = [
            browser_file_input(name, file_bytes)
            for name, file_bytes in selected_bytes
        ]
        try:
        ...
        batch_already_started = bool(
            ...
            and existing_batch_result["manifest"]["batch_id"]
        ...
        if st.button(
            ...
            try:
                ...
                st.error(
                    f"Atlas could not complete the batch import: {error}"
                )

        batch_result = st.session_state.get("batch_result")
        if batch_result and batch_result["manifest"]["batch_id"] == st.session_state.batch_id:
            st.markdown("#### Import report")
            st.markdown(batch_result["report"])
            st.caption(
                "Open the Library workspace to reprocess or remove each "
                "successful source individually."
            )
            if any(
                record.get("terminal_result") == "failed"
                and record.get("retryable")
                for record in batch_result["manifest"]["files"]
            ) and st.button("Retry failed files"):
                try:
