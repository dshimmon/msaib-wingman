...
def render_current_mission(missions: list[Record]) -> str:
    ...
    if primary is None:
        ...
        lines.extend([
            ...
            f"- Next strategic mission gate: {metadata['next_gate']}",
        ])
    lines.extend([
    ...
    if not active:
        lines.append(
            "No active mission workstreams are recorded. Bounded tasks may still "
            "operate concurrently outside mission metadata."
        )
    else:
        lines.extend([
...
def render_context(missions: list[Record]) -> str:
    ...
    lines = [
        ...
        "- Command structure: Maverick is CEO; Goose/Mission Control is COO; Codex is Quarterback and repository operator.",
        "- First repository read: `AGENTS.md`",
        "- Generated-view boundary: mission status only; not implementation permission or a completion gate.",
    ]
    if primary is None:
        lines.extend([
    ...
    lines.extend([
        "- Bounded tasks: may proceed concurrently under `AGENTS.md` and a valid bounded task authority.",
        "- Last completed recorded strategic mission: "
        f"`{latest.metadata['id']}` at "
        f"`{latest.metadata['implementation_commits'][-1]}`",
        f"- Next strategic mission gate: {primary.metadata['next_gate'] if primary else 'Maverick selects the mission; Goose issues the bounded authority.'}",
        "",
    ])
    return "\n".join(lines)
